...
public class AssWorksta1{
    public static void main(String[] args){
        ...
        for (int i=0; i<n; i++){
            ...
            if (unlockedStations.isEmpty() && lockedStations.isEmpty()){ // no station is currently in use
                unlockedStations.add(new Station(researchers[i].arrTime, researchers[i].stayTime, m)); // set one station to unlocked
                isAssigned = true;
            }else if (unlockedStations.isEmpty() &&!lockedStations.isEmpty()){ // there are locked stations
                Station curr = lockedStations.poll(); // use(unlock) one locked station
                curr.use(researchers[i].arrTime, researchers[i].stayTime, m);
                ...
                if(!isAssigned){ // if researcher is not assigned a station
                    if (!lockedStations.isEmpty()){ // if there is locked station 
                        Station curr = lockedStations.poll(); // unlock one locked station and use it
                        curr.use(researchers[i].arrTime, researchers[i].stayTime, m);
                        unlockedStations.add(curr);
                    }else{ // if there is no locked station, unlock a new station
